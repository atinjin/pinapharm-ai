...
- 🧠 **RAG** — 제품 **의미검색**(동의어 사전에 없는 표현도 매칭) + 식약처 **원료 지식 그라운딩**(기능성·주의사항·상호작용)
...
- 🗂️ **약사 어드민** — 상품 관리(검색·정렬·페이징·등록 모달·멀티 삭제), 에이전트 설정(시스템 프롬프트·페르소나·응급 메시지), 상담 스킬 등록(Claude Code 스킬 모델)
...
- 어드민 인증 없음 · 실제 결제 없음(주문 기록만)
- RAG 원료 코퍼스는 약사 검수 시드(확장 예정) · eval·멀티에이전트는 구조만 열어둠
